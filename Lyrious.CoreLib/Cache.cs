...
public class Cache
{
    ...
    public static TEntity? Get<TEntity>(Guid id) where TEntity : EntityBase => Cache<TEntity>.Instance.Get(id);
    public static IEnumerable<TEntity> GetAll<TEntity>() where TEntity : EntityBase => Cache<TEntity>.Instance.GetAll();
    public static IEnumerable<TEntity> GetAllFrom<TEntity>(DateTime from) where TEntity : EntityBase => Cache<TEntity>.Instance.GetAllFrom(from);
    public static TEntity? Remove<TEntity>(Guid id) where TEntity : EntityBase => Cache<TEntity>.Instance.Remove(id);
    public static void Update<TEntity>(TEntity[] values, bool noEvent = false) where TEntity : EntityBase => Cache<TEntity>.Instance.Update(values, noEvent);
    ...
    public static void Bind<TEntity>(IRepository<TEntity> repository) where TEntity : EntityBase => Cache<TEntity>.Instance.Bind(repository);
}
...
public class Cache<TEntity> : Cache, IRepository<TEntity> where TEntity : EntityBase
{
    ...
    private void OnChanged(Changed changed, IEnumerable<TEntity> changedEntities) => Changed?.Invoke(this, new ChangedArgs<TEntity>(changed, changedEntities));
    ...
    public TEntity? Get(Guid id) => entities.TryGetValue(id, out var value) ? value : null;
    public IEnumerable<TEntity> GetAll() => entities.Values;
    public IEnumerable<TEntity> GetAllFrom(DateTime from) => entities.Values.Where(o => o.ChangedAt > from);
    public TEntity? Remove(Guid id)
    {
        ...
        OnChanged(Enums.Changed.Removed, [removedEntity]);
        return removedEntity;
    }
    public TEntity Remove(TEntity entity)
    {
        entities.Remove(entity.Id);
        OnChanged(Enums.Changed.Removed, [entity]);
        return entity;
    }
    ...
    public IList<TEntity> Remove(IEnumerable<TEntity> pendingRemovedEntities, bool noEvent = false)
    {
        ...
        if (!noEvent)
            OnChanged(Enums.Changed.Removed, pendingRemovedEntities);
    ...
    public IList<TEntity> Update(IEnumerable<TEntity> updatedEntities, bool noEvent = false)
    {
        ...
        if (!noEvent)
            OnChanged(Enums.Changed.Updated, changedItems);
    ...
    public IList<TEntity> Add(IEnumerable<TEntity> addedEntities, bool noEvent = false)
    {
        ...
        if (!noEvent)
            OnChanged(Enums.Changed.Added, list);

        return list;
    }

    public void Update(Guid id, TEntity value) => entities.Add(id, value);
    ...
    public void Change(object sender, ChangedArgs<TEntity> args)
    {
        switch (args.Changed)
        {
            case Enums.Changed.Added:
            case Enums.Changed.Insert:
                Add(args.Values);
                break;
            case Enums.Changed.Updated:
                break;
            case Enums.Changed.Removed:
                Remove(args.Values);
                break;
            case Enums.Changed.Cleared:
                break;
            default:
